...
noise = (-0.1, 0.5)
input_size = 300  # Adjust as necessary
...
cwd = os.getcwd()
output_file = os.path.join(cwd, "data", f"new_simul_{nb_triggers}.csv")
...
def generate_input(input_dim=1, input_size=300, nb_triggers=10, trigger_length=10):
    ...
    tics = find_pattern(pattern, U)
    return U, [x + len(pattern) - 1 for x in tics]
...
def run_simulation(U, tics, M, A_default, cycles_list):
    """
    ...
    Args:
        ...
        tics (list): Positions where trigger patterns end.
    ...
    for nb_iter in tqdm(range(len(U) - 2)):
        ...
        memory_length = memory_length + memory if nb_iter in tics else max(1, memory_length - 1)
        ...
        history, _, matrices = simulation(M[0], M[1], M[2], M[3], M[4], current_input,
                                          epoch=len(current_input), stdp=[A_default, eta, plumb, bounds])
